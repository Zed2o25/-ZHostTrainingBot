...
def get_day_completion_stats(user_id, day_num):
    ...
    # Count completed tasks (works for both list and set)
...
def mark_task_completed(user_id, day_num, task_num, task_type):
    ...
    if day_num not in progress["completed_exercises"]:
        progress["completed_exercises"][day_num] = []  # Initialize as list
    
    # Mark task as completed
    task_key = f"{task_type}_{day_num}_{task_num}"
    if task_key not in progress["completed_exercises"][day_num]:
        progress["completed_exercises"][day_num].append(task_key)  # Use append for list
...
Choose from the menu below to start your journey! 🚀"""
    ...
    def show_vocal_task_details(self, chat_id, user_id, task_id):
        ...
        # Get the text to read (if available)
        text_to_read = ""
        if 'text_ar' in task_data and language == 'ar':
            text_to_read = f"\n📖 **النص للقراءة:**\n{task_data['text_ar']}\n"
        elif 'text_en' in task_data and language == 'en':
            text_to_read = f"\n📖 **Text to Read:**\n{task_data['text_en']}\n"
        
        if language == 'ar':
            message = f"🎤 **{task_name}**\n\n"
            message += f"📝 **الوصف:**\n{description}\n"
            message += text_to_read  # Add the text to read here
            message += "\n🎯 **معايير التقييم:**\n"
